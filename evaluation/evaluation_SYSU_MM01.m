...
for i_cam=1:length(load_cam_list)
    ...
    load_name=[feature_name 'cam' num2str(cam_label) '.mat'];
    content=load(fullfile(feature_dir,load_name));
    feature_cam{cam_label}=content.feature_test;
    feature_cam{cam_label}=feature_cam{cam_label}';
    Y{cam_label}=(1:length(content.feature_test));
end
clear content
